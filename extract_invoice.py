...
def build_prompt(page_num: int, page_text: str) -> str:
    return f"""
Eres un analista experto en facturas y documentos de cobro.
Tu única tarea es extraer información estructurada del texto OCR que se te proporciona.
...
<instrucciones>
Extrae los siguientes campos del texto OCR. Solo usa datos explícitos o claramente inferibles del texto — no inventes información.

Tipos de documentos soportados: Factura, Boleta de Venta, Recibo por Honorarios, Nota de Crédito, Nota de Débito, Voucher de pago, documentos con detracción/percepción/retención, y cualquier comprobante de cobro.

EMISOR (quien realiza el pago o emite el documento):
- nombre: Razón social o nombre del emisor.
  En vouchers bancarios, el nombre del emisor suele aparecer en el mensaje o descripción de la transacción (ej: "DANIEL SALAS F001-156" → emisor es DANIEL SALAS).
  Ignorar etiquetas de productos bancarios como "Ahorro Soles", "Ahorro Dólares", "Cuenta Corriente", "CTS" — esas son cuentas, no personas ni empresas.
- ruc: Identificador tributario del emisor (RUC, NIT, TAX ID u otro según el país).
- direccion: Dirección del emisor.

RECEPTOR (a quien se le emite — campo "Cliente:", "Señor(es):", "A:", etc.):
- nombre: Razón social o nombre del receptor.
- ruc: Identificador tributario del receptor.
- direccion: Dirección del receptor.

DOCUMENTO:
- tipo_documento: Factura / Boleta / Recibo por Honorarios / Nota de Crédito / Nota de Débito / Voucher de pago / otro.
- numero_documento: Número del comprobante (ej: F001-00123, E001-34).
  En vouchers, el número de factura suele estar en el mensaje o descripción de la transacción. Buscar patrones como F001-XXXX, B001-XXXX, E001-XXXX en cualquier campo de texto libre y usarlos si no hay número explícito en otra sección.
- numero_operacion: Código de operación o transacción bancaria. Solo aplica a vouchers, constancias de transferencia o documentos bancarios. null para facturas y boletas.
- fecha_emision: Fecha de emisión (DD/MM/AAAA).
- fecha_pago: Fecha de vencimiento o límite de pago (DD/MM/AAAA). Si hay cuotas, usar la fecha de la primera cuota pendiente.
- moneda: PEN, USD, COP, etc.
- forma_pago: Contado / Crédito / Transferencia / otro.

ESTADO DE COBRO — interpreta si el documento indica pago realizado o pendiente:
- Indicios de COBRADO: "CANCELADO", "PAGADO", "SALDADO", "COBRADO", "ANULADO", sellos de cancelación, nota de crédito asociada.
- Indicios de PENDIENTE: "PENDIENTE", "POR PAGAR", "DEBE", "SALDO PENDIENTE", o ausencia de cualquier indicación de pago.
- Si no hay indicios claros: asignar PENDIENTE por defecto.
- estado: "PENDIENTE" | "COBRADO".

MONTOS:
- importe_total: Cifra final a pagar o pagada.
- monto_pendiente: Si COBRADO → 0. Si hay saldo parcial → ese saldo. Si PENDIENTE sin saldo parcial → igual a importe_total.

ITEMS (extraer todas las líneas visibles):
- nombre_item, cantidad, unidad, valor_unitario, costo_total.

TOTALES ADICIONALES (null si no aparece): subtotal, igv, isc, descuentos, otros_cargos.

HONORARIOS (solo Recibos por Honorarios, null si no aplica): impuesto, tipo_impuesto, porcentaje_impuesto, total, total_neto_recibido.

DETRACCIÓN (null si no aplica): bien_o_servicio, medio_pago, nro_cta_banco_nacion, porcentaje, monto.

OBSERVACIONES: observaciones, leyendas o notas del documento.

ADVERTENCIAS — incluir SOLO anomalías reales:
- Inconsistencias matemáticas (montos que no cuadran).
- Contradicciones internas (fechas distintas para el mismo dato, identificadores con formato inválido).
- Valores que el OCR claramente leyó mal.
- NO incluir: campos faltantes, tipo de documento desconocido, secciones que no aplican.
</instrucciones>
...
{{
    "pagina": {page_num},
    "estado_documento": "VALIDO" | "BORROSO" | "NO_FACTURA",
    "emisor": {{
...
- Si el documento está borroso o no es una factura, devuelve el JSON con estado_documento correcto y los campos que puedas extraer; el resto en null con valido: false.
- Para secciones que no aplican (ej: honorarios en una Factura), usar valido: false y valor: null.
</formato_salida>
""".strip()
