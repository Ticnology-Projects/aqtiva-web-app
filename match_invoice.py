...
def guardar_conciliacion_s3(s3_key: str, resultado: dict):
    """
    Toma la s3_key original (ej: output/archivo_20260513215308_4a0f70f5.json)
    y guarda el resultado refinado en processed/archivo_20260513215308_4a0f70f5.json
    """
    # Extraemos el nombre del archivo (ej: archivo_20260513215308_4a0f70f5.json)
    nombre_archivo = os.path.basename(s3_key)
    processed_key = f"processed/{nombre_archivo}"
    ...
    try:
        ...
        print(f"[match_invoice] Estado guardado exitosamente en: s3://{BUCKET_NAME}/{processed_key}")
        return processed_key
    except Exception as e:
        print(f"[match_invoice] Error crítico al persistir en S3: {e}")
        raise
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if not candidates:
            ...
            # Guardamos el reporte de falla para que el frontend sepa que ya fue auditado
            guardar_conciliacion_s3(s3_key, respuesta_sin_match)
            return _ok(respuesta_sin_match)

        # 4. Claude evalúa candidatos y elige el mejor match
        conciliacion = evaluate(extraccion, candidates)

        # Construimos el objeto de respuesta completo
        respuesta_final = {
            "s3_key":        s3_key,
            "query_usada":   query,
        ...
        # == NUEVO: PERSISTENCIA AUTOMÁTICA EN LA CARPETA PROCESSED ==
        guardar_conciliacion_s3(s3_key, respuesta_final)

        return _ok(respuesta_final)
